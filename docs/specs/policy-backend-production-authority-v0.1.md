...
3. Provider bundle binding presence, every non-null provider binding field
   emitted by the v0.1 builder, and fail-closed replay of the supplied
   `--provider-bundle` through `policy-backend-provider-export-bundle-verify`.
4. Optional service bundle bindings and every non-null service binding field
   ...
   hash. Omitted optional service sources may produce replay warnings, but they
