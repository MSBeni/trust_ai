...
- `external-evidence`: customer-owned evidence references for paid pilots, signed value, and external scrutiny survival.

The dossier never stores raw customer contracts, payment records, private proof packs, or customer data. It stores references, hashes, status values, counters, and source-artifact hashes.
...
- `metrics`: derived counts and signed value.
...
- `contract_evidence_ref`, required for signed-value controls in `external-evidence` mode
...
- `evidence_ref`, required for scrutiny-survival controls in `external-evidence` mode
...
- metrics do not match partner and scrutiny records.
...
- `signed-value-target`: in `external-evidence` mode, signed/active partner values total at least $250k and each partner has `contract_evidence_ref`.
- `external-scrutiny-survival-target`: in `external-evidence` mode, at least one survived/accepted scrutiny event has `evidence_ref`.
...
- `raw-customer-data-excluded`: the dossier excludes raw customer data and private contracts.
...
- partner counts and signed pilot value
- external scrutiny survival count
