...
- `image_signature`: replay result for the structured image signature artifact, including subject hash, verification status, and errors.
- `artifacts`: exactly one SBOM artifact hash record, one provenance artifact hash record, and one structured signature artifact hash record.
...
- SBOM, provenance, and signature artifact files exist and match their recorded
  hashes and sizes.
- the signature artifact is valid `trustai.deployment-image-signature/0.1`
  JSON, its subject exactly matches the replayed image digest, pinned reference,
  deployment manifest hash, SBOM hash, and provenance hash, and its detached
  signature verifies.
...
- the image signature verification check passes; placeholder or malformed
  signature files fail the receipt.
...
manifest binding, image-signature verification result, artifact count, source count, check summary, and pass status.
...
A production release pipeline should replace local development signatures with
