...
- `authority_evidence`: external evidence references for covered checklist items, including derived `source_context` bindings.
...
Each evidence item contains `requirement_id`, `authority_kind`, `evidence_ref`, `evidence_hash`, `description`, optional `issuer`, `subject`, `source_uri`, `issued_at`, `expires_at`, derived `source_context`, and derived `evidence_id`. The derived `source_context` binds each external authority row to the service attestation id/hash, environment, service/provider endpoint, source delivery, dispatch worker, provider credential ref, service audit root, worker operation IDs/hashes/run refs, request/response hashes, delivery/provider/audit roots, worker bundle IDs/hashes, bundle delivery hashes, and bundled source roots recorded in the dossier.
...
6. Validate every authority evidence item has an accepted requirement ID, authority kind, hash ref, stable evidence ID, and derived `source_context` matching the service, worker, and worker-bundle bindings.
7. Recompute the production authority summary from `authority_evidence`.
8. Recompute controls from service, worker, bundle, evidence, and summary bindings.
9. Validate freshness metadata when `require_fresh` is set.
10. Reject `production-dossier` mode unless every production authority requirement is covered.
11. Reject secret-like fields unless they are redacted references or hash/root references.
...
The chain payload includes the dossier ID/hash, mode, environment, authority references, service attestation binding, worker receipt bindings, worker bundle bindings, coverage summary, control summary, and compact authority evidence references including `source_context`.

## Limits

This dossier is an authority evidence manifest, not a live provider API client. It does not fetch provider state. It proves signed source-context binding for authority evidence rows, derived control status, coverage accounting, freshness windows, and strict production-claim gates. A production claim is only as strong as the external evidence hashes and retained source artifacts supplied to independent verifiers.
