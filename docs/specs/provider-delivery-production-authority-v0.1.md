...
This specification defines a signed TrustAI dossier that binds provider delivery service and worker receipts to explicit production-authority evidence for credentialed external Slack, GitHub, and GitLab posting.
...
A provider delivery production authority dossier records that boundary. It binds the signed delivery service attestation and one or more signed worker receipts to a checklist of external authority evidence. Verifiers can reject incomplete or stale production claims while still accepting partial provider dossiers as useful evidence.
...
5. Recompute the production authority summary from `authority_evidence`.
6. Validate every authority evidence item has an accepted requirement ID, authority kind, hash ref, and stable evidence ID.
7. Validate freshness metadata when `require_fresh` is set.
8. Reject `production-dossier` mode unless every production authority requirement is covered.
9. Reject secret-like fields unless they are redacted references or hash/root references.

If raw delivery, payload, provider response, provider audit correlation, or provider operations service sources are omitted, the verifier MAY still accept the signed service/worker binding and MUST emit warnings that source hashes were not replayed.
...
The chain payload includes the dossier ID/hash, mode, environment, authority references, service attestation binding, worker receipt bindings, coverage summary, control summary, and authority evidence references.
