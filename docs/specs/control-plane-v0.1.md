...
- `production_replacement_worklist()`: operator summary derived from the latest collection package, including collection readiness, blocked task counts by owner/authority kind/requirement, capped blocked task rows, next actions, and latest closure context.
