...
- `production_replacement_worklist()`: operator summary derived from the latest submission, submission review, collection package, and closure artifacts, including submitted task counts, submitted live/placeholder source URI counts, review readiness, collection readiness, blocked task counts by owner/authority kind/requirement, capped submitted/blocked task rows, next actions, and latest closure context.
