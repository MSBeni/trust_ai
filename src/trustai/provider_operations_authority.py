    ...
    *,
    ...
    if item.get("evidence_hash"):
        try:
            ...
            errors.append(f"invalid provider operations authority evidence: {exc}")
            return "missing"
        if item.get("evidence_hash") != normalized_hash:
            errors.append(f"provider operations authority evidence_hash is not canonical lowercase sha256: {requirement_id}")
