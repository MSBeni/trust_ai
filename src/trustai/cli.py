...
from .framework_runtime_service_provider import (
    append_framework_runtime_service_provider_receipt,
    build_framework_runtime_service_provider_receipt,
    load_framework_runtime_service_provider_export,
    load_framework_runtime_service_provider_receipt,
...
from .framework_runtime_service_authority_worker import (
    ...
    load_framework_runtime_service_authority_worker_receipt,
    verify_framework_runtime_service_authority_worker_receipt,
    write_framework_runtime_service_authority_worker_receipt,
)
from .framework_runtime_service_authority_provider import (
    append_framework_runtime_service_authority_provider_receipt,
    build_framework_runtime_service_authority_provider_receipt,
    load_framework_runtime_service_authority_provider_export,
    load_framework_runtime_service_authority_provider_receipt,
...
from .framework_runtime_service_authority_recorded_export_provider import (
    ...
    load_framework_runtime_service_authority_recorded_export_provider_export,
    load_framework_runtime_service_authority_recorded_export_provider_receipt,
...
def _load_identity_session_sources(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "identity_payload_path": args.identity_payload if args.identity_payload else None,
...
def cmd_framework_runtime_service_worker_append(args: argparse.Namespace) -> int:
    ...
    print(f"chain root: {chain.tree()['root']}")
    return 0
def _load_framework_runtime_service_provider_sources(args: argparse.Namespace, *, require_all: bool) -> dict[str, object]:
    sources: dict[str, object] = {}
    if getattr(args, "provider_export", None):
        sources["provider_export"] = load_framework_runtime_service_provider_export(args.provider_export)
    elif require_all:
        raise ValueError("framework runtime service provider export is required")
    if getattr(args, "service_worker", None):
        sources["service_worker"] = load_framework_runtime_service_worker_receipt(args.service_worker)
    elif require_all:
        raise ValueError("framework runtime service worker receipt is required")
    worker_sources = _load_framework_runtime_service_worker_sources(args, require_all=require_all)
    sources.update(worker_sources)
    return sources


def cmd_framework_runtime_service_provider(args: argparse.Namespace) -> int:
    try:
        sources = _load_framework_runtime_service_provider_sources(args, require_all=True)
        receipt = build_framework_runtime_service_provider_receipt(
            sources["provider_export"],
            sources["service_worker"],
            sources["service_attestation"],
            sources["storage_receipt"],
            sources["storage_export"],
            sources["worker"],
        ...
        result = verify_framework_runtime_service_provider_receipt(receipt, **sources, root=args.root, key=args.key)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service provider receipt generation failed: {exc}", file=sys.stderr)
        return 1
    if not result.ok:
        print("framework runtime service provider receipt generation failed verification", file=sys.stderr)
        for error in result.errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    write_framework_runtime_service_provider_receipt(args.out, receipt)
    print(f"framework runtime service provider receipt: {args.out}")
    print(f"provider receipt id: {receipt['provider_receipt_id']}")
    print(f"worker operation id: {receipt['service_worker_binding']['worker_operation_id']}")
    for warning in result.warnings:
        print(f"warning: {warning}")
    return 0


def cmd_framework_runtime_service_provider_verify(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_provider_receipt(args.receipt)
        sources = _load_framework_runtime_service_provider_sources(args, require_all=False)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service provider receipt verification failed: {exc}", file=sys.stderr)
        return 1
    result = verify_framework_runtime_service_provider_receipt(receipt, **sources, root=args.root, key=args.key)
    if result.ok:
        print(f"verified framework runtime service provider receipt: {args.receipt}")
        print(f"provider receipt id: {receipt['provider_receipt_id']}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"framework runtime service provider receipt verification failed: {args.receipt}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    return 1


def cmd_framework_runtime_service_provider_append(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_provider_receipt(args.receipt)
        sources = _load_framework_runtime_service_provider_sources(args, require_all=True)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service provider receipt append failed: {exc}", file=sys.stderr)
        return 1
    chain = _load_chain(args)
    try:
        entry = append_framework_runtime_service_provider_receipt(chain, receipt, **sources, root=args.root, key=args.key)
    except ValueError as exc:
        print(f"framework runtime service provider receipt append failed: {exc}", file=sys.stderr)
        return 1
    chain.save()
    if args.out:
        _write_json(args.out, entry)
        print(f"framework runtime service provider entry: {args.out}")
    print(f"framework runtime service provider entry id: {entry['entry_id']}")
    print(f"provider receipt id: {receipt['provider_receipt_id']}")
    print(f"chain root: {chain.tree()['root']}")
    return 0
...
def _load_framework_runtime_service_authority_worker_sources(args: argparse.Namespace, *, require_all: bool) -> dict[str, object]:
    sources: dict[str, object] = {}
    dossier_path = getattr(args, "authority_dossier", None) or getattr(args, "dossier", None)
    if dossier_path:
        sources["authority_dossier"] = load_framework_runtime_service_authority_dossier(dossier_path)
    elif require_all:
        raise ValueError("framework runtime service authority dossier is required")
    authority_sources = _load_framework_runtime_service_authority_sources(args, require_all=require_all)
    sources.update(authority_sources)
    return sources


def cmd_framework_runtime_service_authority_worker(args: argparse.Namespace) -> int:
    try:
        sources = _load_framework_runtime_service_authority_worker_sources(args, require_all=True)
        authority_dossier = sources.pop("authority_dossier")
        provider_receipt = sources.pop("provider_receipt")
        receipt = build_framework_runtime_service_authority_worker_receipt(
            ...
            **sources,
            ...
            queue_message_hash=args.queue_message_hash,
            dead_letter_queue_ref=args.dead_letter_queue_ref,
            authority_request_ref=args.authority_request_ref,
            dossier_storage_ref=args.dossier_storage_ref,
            dossier_storage_hash=args.dossier_storage_hash,
            report_storage_ref=args.report_storage_ref,
            report_storage_hash=args.report_storage_hash,
            request_hash=args.request_hash,
            response_status=args.response_status,
            response_hash=args.response_hash,
            metrics_ref=args.metrics_ref,
        ...
        print(f"framework runtime service authority worker receipt generation failed: {exc}", file=sys.stderr)
        return 1
    if not result.ok:
        print("framework runtime service authority worker receipt generation failed verification", file=sys.stderr)
        for error in result.errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    write_framework_runtime_service_authority_worker_receipt(args.out, receipt)
    print(f"framework runtime service authority worker receipt: {args.out}")
    print(f"worker operation id: {receipt['worker_operation_id']}")
    print(f"authority dossier id: {receipt['authority']['dossier_id']}")
    for warning in result.warnings:
        print(f"warning: {warning}")
    return 0


def cmd_framework_runtime_service_authority_worker_verify(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_worker_receipt(args.receipt)
        sources = _load_framework_runtime_service_authority_worker_sources(args, require_all=False)
        authority_dossier = sources.pop("authority_dossier", None)
        provider_receipt = sources.pop("provider_receipt", None)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority worker receipt verification failed: {exc}", file=sys.stderr)
        return 1
    result = verify_framework_runtime_service_authority_worker_receipt(
        receipt,
        authority_dossier=authority_dossier,
    ...
    if result.ok:
        print(f"verified framework runtime service authority worker receipt: {args.receipt}")
        print(f"worker operation id: {receipt['worker_operation_id']}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"framework runtime service authority worker receipt verification failed: {args.receipt}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    return 1


def cmd_framework_runtime_service_authority_worker_append(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_worker_receipt(args.receipt)
        sources = _load_framework_runtime_service_authority_worker_sources(args, require_all=True)
        authority_dossier = sources.pop("authority_dossier")
        ...
        print(f"framework runtime service authority worker receipt append failed: {exc}", file=sys.stderr)
        return 1
    chain = _load_chain(args)
    try:
    ...
    if args.out:
        _write_json(args.out, entry)
        print(f"framework runtime service authority worker entry: {args.out}")
    print(f"framework runtime service authority worker entry id: {entry['entry_id']}")
    print(f"worker operation id: {receipt['worker_operation_id']}")
    print(f"chain root: {chain.tree()['root']}")
    return 0

def _load_framework_runtime_service_authority_provider_sources(args: argparse.Namespace, *, require_all: bool) -> dict[str, object]:
    sources: dict[str, object] = {}
    if getattr(args, "authority_provider_export", None):
        sources["authority_provider_export"] = load_framework_runtime_service_authority_provider_export(args.authority_provider_export)
    elif require_all:
        raise ValueError("framework runtime service authority provider export is required")
    if getattr(args, "authority_worker", None):
        sources["authority_worker"] = load_framework_runtime_service_authority_worker_receipt(args.authority_worker)
    elif require_all:
        raise ValueError("framework runtime service authority worker receipt is required")
    worker_sources = _load_framework_runtime_service_authority_worker_sources(args, require_all=require_all)
    sources.update(worker_sources)
    return sources


def _framework_runtime_service_authority_provider_verify_kwargs(sources: dict[str, object]) -> dict[str, object]:
    return {
...
def cmd_framework_runtime_service_authority_provider(args: argparse.Namespace) -> int:
    try:
        sources = _load_framework_runtime_service_authority_provider_sources(args, require_all=True)
        receipt = build_framework_runtime_service_authority_provider_receipt(
            sources["authority_provider_export"],
            sources["authority_worker"],
            sources["authority_dossier"],
        ...
        result = verify_framework_runtime_service_authority_provider_receipt(
            receipt,
            **_framework_runtime_service_authority_provider_verify_kwargs(sources),
            root=args.root,
            key=args.key,
        )
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority provider receipt generation failed: {exc}", file=sys.stderr)
        return 1
    if not result.ok:
        print("framework runtime service authority provider receipt generation failed verification", file=sys.stderr)
        for error in result.errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    write_framework_runtime_service_authority_provider_receipt(args.out, receipt)
    print(f"framework runtime service authority provider receipt: {args.out}")
    print(f"provider receipt id: {receipt['provider_receipt_id']}")
    print(f"authority worker operation id: {receipt['authority_worker_binding']['worker_operation_id']}")
    for warning in result.warnings:
        print(f"warning: {warning}")
    return 0


def cmd_framework_runtime_service_authority_provider_verify(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_provider_receipt(args.receipt)
        sources = _load_framework_runtime_service_authority_provider_sources(args, require_all=False)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority provider receipt verification failed: {exc}", file=sys.stderr)
        return 1
    result = verify_framework_runtime_service_authority_provider_receipt(
        receipt,
        **_framework_runtime_service_authority_provider_verify_kwargs(sources),
    ...
    if result.ok:
        print(f"verified framework runtime service authority provider receipt: {args.receipt}")
        print(f"provider receipt id: {receipt['provider_receipt_id']}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"framework runtime service authority provider receipt verification failed: {args.receipt}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    return 1


def cmd_framework_runtime_service_authority_provider_append(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_provider_receipt(args.receipt)
        sources = _load_framework_runtime_service_authority_provider_sources(args, require_all=True)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority provider receipt append failed: {exc}", file=sys.stderr)
        return 1
    chain = _load_chain(args)
    try:
...
def cmd_framework_runtime_service_authority_recorded_export_provider(args: argparse.Namespace) -> int:
    try:
        provider_export = load_framework_runtime_service_authority_recorded_export_provider_export(args.recorded_export_provider_export)
        recorded_export_worker = load_framework_runtime_service_authority_recorded_export_worker_receipt(args.recorded_export_worker)
        recorded_export = load_framework_runtime_service_authority_recorded_export(args.recorded_export)
        sources = _load_framework_runtime_service_authority_recorded_export_sources(args, require_all=True)
        artifact_paths = _framework_runtime_service_authority_recorded_export_artifact_paths(args, require_all=True)
        ...
        print(f"framework runtime service authority recorded export provider generation failed: {exc}", file=sys.stderr)
        return 1
    if not result.ok:
        print("framework runtime service authority recorded export provider generation failed verification", file=sys.stderr)
        for error in result.errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    write_framework_runtime_service_authority_recorded_export_provider_receipt(args.out, receipt)
    print(f"framework runtime service authority recorded export provider receipt: {args.out}")
    print(f"provider receipt id: {receipt['provider_receipt_id']}")
    print(f"recorded export worker operation id: {receipt['recorded_export_worker_binding']['worker_operation_id']}")
    print(f"provider export root: {receipt['provider_export']['storage_record_root']}")
    for warning in result.warnings:
        print(f"warning: {warning}")
    return 0


def cmd_framework_runtime_service_authority_recorded_export_provider_verify(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_recorded_export_provider_receipt(args.receipt)
        provider_export = load_framework_runtime_service_authority_recorded_export_provider_export(args.recorded_export_provider_export) if args.recorded_export_provider_export else None
        recorded_export_worker = load_framework_runtime_service_authority_recorded_export_worker_receipt(args.recorded_export_worker) if args.recorded_export_worker else None
        recorded_export = load_framework_runtime_service_authority_recorded_export(args.recorded_export) if args.recorded_export else None
        sources = _load_framework_runtime_service_authority_recorded_export_sources(args, require_all=False)
        artifact_paths = _framework_runtime_service_authority_recorded_export_artifact_paths(args, require_all=False)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority recorded export provider verification failed: {exc}", file=sys.stderr)
        return 1
    result = verify_framework_runtime_service_authority_recorded_export_provider_receipt(
        receipt,
        provider_export=provider_export,
    ...
    if result.ok:
        print(f"verified framework runtime service authority recorded export provider receipt: {args.receipt}")
        print(f"provider receipt id: {receipt['provider_receipt_id']}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"framework runtime service authority recorded export provider verification failed: {args.receipt}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    return 1


def cmd_framework_runtime_service_authority_recorded_export_provider_append(args: argparse.Namespace) -> int:
    try:
        receipt = load_framework_runtime_service_authority_recorded_export_provider_receipt(args.receipt)
        provider_export = load_framework_runtime_service_authority_recorded_export_provider_export(args.recorded_export_provider_export)
        recorded_export_worker = load_framework_runtime_service_authority_recorded_export_worker_receipt(args.recorded_export_worker)
        ...
        print(f"framework runtime service authority recorded export provider append failed: {exc}", file=sys.stderr)
        return 1
    chain = _load_chain(args)
    try:
    ...
    if args.out:
        _write_json(args.out, entry)
        print(f"framework runtime service authority recorded export provider entry: {args.out}")
    print(f"framework runtime service authority recorded export provider entry id: {entry['entry_id']}")
    print(f"provider receipt id: {receipt['provider_receipt_id']}")
    print(f"chain root: {chain.tree()['root']}")
    return 0

def cmd_framework_runtime_service_authority_recorded_export_provider_bundle(args: argparse.Namespace) -> int:
    try:
        provider_receipt = load_framework_runtime_service_authority_recorded_export_provider_receipt(args.receipt)
        provider_export = load_framework_runtime_service_authority_recorded_export_provider_export(args.recorded_export_provider_export)
        recorded_export_worker = load_framework_runtime_service_authority_recorded_export_worker_receipt(args.recorded_export_worker)
        recorded_export = load_framework_runtime_service_authority_recorded_export(args.recorded_export)
        sources = _load_framework_runtime_service_authority_recorded_export_sources(args, require_all=True)
        ...
        print(f"framework runtime service authority recorded export provider bundle generation failed: {exc}", file=sys.stderr)
        return 1
    if not result.ok:
        print("framework runtime service authority recorded export provider bundle generation failed verification", file=sys.stderr)
        for error in result.errors:
            print(f"- {error}", file=sys.stderr)
        return 1
...
def cmd_framework_runtime_service_authority_recorded_export_provider_bundle_verify(args: argparse.Namespace) -> int:
    try:
        bundle = load_framework_runtime_service_authority_recorded_export_provider_bundle(args.bundle)
    except (OSError, ValueError) as exc:
        print(f"framework runtime service authority recorded export provider bundle verification failed: {exc}", file=sys.stderr)
        return 1
    result = verify_framework_runtime_service_authority_recorded_export_provider_bundle(bundle, key=args.key)
    if result.ok:
        print(f"verified framework runtime service authority recorded export provider bundle: {args.bundle}")
        print(f"bundle id: {bundle['bundle_id']}")
        print(f"provider receipt id: {bundle['source']['provider_receipt_id']}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"framework runtime service authority recorded export provider bundle verification failed: {args.bundle}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    return 1
...
def cmd_framework_runtime_service_authority_recorded_export_provider_bundle_append(args: argparse.Namespace) -> int:
    try:
        ...
        print(f"framework runtime service authority recorded export provider bundle append failed: {exc}", file=sys.stderr)
        return 1
    chain = _load_chain(args)
    try:
        entry = append_framework_runtime_service_authority_recorded_export_provider_bundle(
            chain,
            bundle,
            key=args.key,
        )
    ...
    if args.out:
        _write_json(args.out, entry)
        print(f"framework runtime service authority recorded export provider bundle entry: {args.out}")
    print(f"framework runtime service authority recorded export provider bundle entry id: {entry['entry_id']}")
    print(f"bundle id: {bundle['bundle_id']}")
    print(f"chain root: {chain.tree()['root']}")
    return 0
...
def cmd_roadmap_evidence_report_verify(args: argparse.Namespace) -> int:
    chain = _load_chain(args)
    report = load_roadmap_evidence_report(args.report)
    result = verify_roadmap_evidence_report(
        report,
        chain,
        key=args.key,
    ...
    if result.ok:
        ...
        print(f"tree root: {report.get('chain', {}).get('tree', {}).get('root')}")
        for warning in result.warnings:
            print(f"warning: {warning}")
        return 0
    print(f"roadmap evidence report verification failed: {args.report}", file=sys.stderr)
    for error in result.errors:
        print(f"- {error}", file=sys.stderr)
    for warning in result.warnings:
        print(f"warning: {warning}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def _add_framework_runtime_service_authority_worker_fields(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--queue-message-hash", required=True)
        parser.add_argument("--dead-letter-queue-ref")
        parser.add_argument("--authority-request-ref", required=True)
        parser.add_argument("--dossier-storage-ref", required=True)
        parser.add_argument("--dossier-storage-hash", required=True)
        parser.add_argument("--report-storage-ref")
        parser.add_argument("--report-storage-hash")
        parser.add_argument("--request-hash")
        parser.add_argument("--response-status", type=int)
        parser.add_argument("--response-hash")
        parser.add_argument("--metrics-ref", required=True)
    ...
    roadmap_evidence_report_verify = subparsers.add_parser("roadmap-evidence-report-verify", help="verify an offline roadmap evidence report against an evidence chain")
    roadmap_evidence_report_verify.add_argument("report")
    ...
    roadmap_evidence_bundle.add_argument("--require-source-artifacts", action="store_true", help="fail export unless bundled chain audits, manifests, and manifest evidence files are embedded")
