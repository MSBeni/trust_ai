...
from .insurer_partner_worker_bundle import verify_insurer_partner_worker_bundle
...
def build_insurer_partner_authority_dossier(
    ...
    *,
    ...
    for bundle in bundles:
        bundle_result = verify_insurer_partner_worker_bundle(bundle, key=key)
        if not bundle_result.ok:
            raise ValueError("invalid insurer partner worker bundle source: " + "; ".join(bundle_result.errors))
    ...
    if link_errors:
        raise ValueError("invalid insurer partner worker bundle binding: " + "; ".join(link_errors))
...
def verify_insurer_partner_authority_dossier(
    ...
    *,
    ...
    if not isinstance(dossier.get("controls"), list) or not dossier.get("controls"):
        ...
        errors.append("insurer partner authority controls do not match dossier body")
...
def _worker_bundle_binding(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "telemetry_hash": source.get("telemetry_hash"),
        "underwriting_quote_hash": source.get("underwriting_quote_hash"),
        ...
        "partner_event_log_root": source.get("partner_event_log_root"),
        "policy_binding_ref": source.get("policy_binding_ref"),
        "policy_binding_hash": source.get("policy_binding_hash"),
        "pack_id": source.get("pack_id"),
        "contract_id": source.get("contract_id"),
        "consent_id": source.get("consent_id"),
        "underwriter": source.get("underwriter"),
        "quote_id": source.get("quote_id"),
        "risk_tier": source.get("risk_tier"),
        "quoted_premium_usd": source.get("quoted_premium_usd"),
        "discount_percent": source.get("discount_percent"),
        "coverage_limit_usd": source.get("coverage_limit_usd"),
        ...
        "actuarial_product_replayed": summary.get("actuarial_product_replayed"),
        "actuarial_corpus_count": summary.get("actuarial_corpus_count"),
...
def _worker_bundle_link_errors(bundle_bindings: list[dict[str, Any]], service_binding: Any, worker_bindings: Any) -> list[str]:
    ...
    telemetry_hashes = {binding.get("telemetry_hash") for binding in worker_bindings if isinstance(binding, dict) and binding.get("telemetry_hash")} if isinstance(worker_bindings, list) else set()
    for binding in bundle_bindings:
        if not isinstance(binding, dict):
            continue
        if service_hash and binding.get("service_attestation_hash") != service_hash:
            errors.append("insurer partner authority worker bundle does not reference supplied service attestation hash")
        if service_id and binding.get("service_attestation_id") != service_id:
            errors.append("insurer partner authority worker bundle does not reference supplied service attestation id")
        if worker_ids and binding.get("worker_operation_id") not in worker_ids:
            errors.append("insurer partner authority worker bundle does not reference a supplied worker receipt id")
        if worker_hashes and binding.get("worker_receipt_hash") not in worker_hashes:
            errors.append("insurer partner authority worker bundle does not reference a supplied worker receipt hash")
        if telemetry_hashes and binding.get("telemetry_hash") not in telemetry_hashes:
            errors.append("insurer partner authority worker bundle does not reference a supplied telemetry hash")
    return errors
...
def _verify_worker_bundle_bindings(bindings: Any, worker_bundles: list[dict[str, Any]] | None, service_binding: Any, worker_bindings: Any, key: str | None, errors: list[str], warnings: list[str]) -> None:
    if bindings is None:
        if worker_bundles:
            errors.append("insurer partner authority worker_bundle_bindings are required when worker bundles are supplied")
        return
    if not isinstance(bindings, list):
        errors.append("insurer partner authority worker_bundle_bindings must be a list")
        return
    for binding in bindings:
        if not isinstance(binding, dict):
            errors.append("insurer partner authority worker_bundle_binding must be an object")
            continue
        for field in ("bundle_id", "bundle_hash", "bundle_schema", "bundle_mode", "environment", "generated_at", "reviewer_ref", "bundle_ref", "worker_operation_id", "worker_receipt_hash", "service_attestation_id", "service_attestation_hash", "telemetry_hash", "underwriting_quote_hash", "run_ref", "operation_kind", "destination_ref", "response_status", "partner_event_log_root", "policy_binding_ref", "policy_binding_hash", "pack_id", "contract_id", "consent_id", "underwriter", "quote_id", "risk_tier", "quoted_premium_usd", "discount_percent", "coverage_limit_usd", "source_artifact_count", "source_artifact_sha256_root", "source_artifact_content_root", "frontend_bundle_replayed", "actuarial_product_replayed", "actuarial_corpus_count", "worker_control_summary"):
            if binding.get(field) in (None, "", []):
                errors.append(f"insurer partner authority worker_bundle_binding.{field} is required")
    errors.extend(_worker_bundle_link_errors(bindings, service_binding, worker_bindings))
    if not worker_bundles:
        if bindings:
            warnings.append("insurer partner authority worker bundles were not supplied; worker bundle hashes were not replayed")
        return
    expected = [_worker_bundle_binding(bundle) for bundle in worker_bundles]
    if bindings != expected:
        errors.append("insurer partner authority worker_bundle_bindings do not match supplied worker bundles")
    for bundle in worker_bundles:
        result = verify_insurer_partner_worker_bundle(bundle, key=key)
        if not result.ok:
            errors.extend(f"insurer partner authority worker bundle source: {error}" for error in result.errors)
        warnings.extend(f"insurer partner authority worker bundle source: {warning}" for warning in result.warnings)
...
def _verify_service_attestation_binding(binding: Any, service_attestation: dict[str, Any] | None, sources: dict[str, Any], key: str | None, errors: list[str], warnings: list[str]) -> None:
    ...
    if not isinstance(binding, dict):
        ...
        return
    for field in (
        "attestation_id", "attestation_hash", "attestation_schema", "attestation_mode", "environment", "attested_at", "source_count", "source_hash", "source_schemas", "required_source_types", "service_ref", "service_kind", "service_version", "endpoint_url", "partner_api_endpoint", "service_image_digest", "service_binary_hash", "frontend_bundle_ref", "frontend_bundle_hash", "frontend_bundle_artifact_hash", "api_ref", "queue_ref", "policy_system_ref", "replicas_min", "replicas_max", "availability_zones", "underwriter", "quote_id", "quote_ref", "quote_product", "partner_contract_ref", "partner_policy_system_ref", "actuarial_product_id", "telemetry_hash", "telemetry_schema", "consent_id", "consent_active", "pack_id", "contract_id", "risk_score", "risk_tier", "gate_outcome", "quoted_premium_usd", "discount_percent", "coverage_limit_usd", "auth_provider_ref", "partner_auth_policy_ref", "rbac_policy_ref", "consent_policy_ref", "data_minimization_policy_ref", "pii_redaction_policy_ref", "tenant_isolation_ref", "rate_limit_policy_ref", "request_signing_ref", "network_policy_ref", "egress_policy_ref", "encryption_key_ref", "audit_log_ref", "audit_log_root", "access_log_ref", "access_log_root", "delivery_log_ref", "delivery_log_root", "metrics_ref", "alert_policy_ref", "retention_until", "actor_ref", "credential_ref", "partner_credential_ref", "evidence_refs"
    ):
        if binding.get(field) in (None, "", []):
...
def _verify_worker_receipt_bindings(bindings: Any, worker_receipts: list[dict[str, Any]] | None, service_attestation: dict[str, Any] | None, sources: dict[str, Any], key: str | None, errors: list[str], warnings: list[str]) -> None:
    ...
    for binding in bindings:
        if not isinstance(binding, dict):
            ...
            continue
        for field in (
            "worker_operation_id", "worker_operation_hash", "receipt_schema", "receipt_mode", "environment", "recorded_at", "source_count", "source_hash", "source_schemas", "required_source_types", "service_attestation_id", "service_attestation_hash", "service_ref", "service_kind", "service_version", "endpoint_url", "partner_api_endpoint", "partner_contract_ref", "policy_system_ref", "queue_ref", "pack_id", "contract_id", "consent_id", "consent_active", "telemetry_hash", "quote_id", "quote_ref", "underwriter", "risk_score", "risk_tier", "quoted_premium_usd", "discount_percent", "coverage_limit_usd", "worker_ref", "run_ref", "operation_kind", "actor_ref", "started_at", "completed_at", "attempt", "max_attempts", "success", "schedule_ref", "cadence_seconds", "lease_ref", "checkpoint_ref", "checkpoint_hash", "previous_cursor_ref", "next_cursor_ref", "next_run_at", "delivery_queue_ref", "queue_message_ref", "destination_ref", "delivery_log_ref", "delivery_log_root", "partner_event_log_ref", "partner_event_log_root", "request_hash", "response_status", "response_hash", "worker_policy_system_ref", "policy_workflow_ref", "policy_workflow_hash", "policy_binding_ref", "policy_binding_hash", "workflow_status", "metrics_ref", "audit_log_ref", "audit_log_root", "access_log_ref", "access_log_root", "retention_until", "evidence_refs", "credential_ref", "partner_credential_ref"
        ):
            if binding.get(field) in (None, "", []):
...
def _verify_authority_evidence_item(item: dict[str, Any], errors: list[str], warnings: list[str], *, now: Any, require_fresh: bool, source_context: dict[str, Any]) -> str:
    ...
    if item.get("evidence_hash"):
        try:
            ...
            errors.append(f"invalid insurer partner authority evidence: {exc}")
            return "missing"
        if item.get("evidence_hash") != normalized_hash:
            errors.append(f"insurer partner authority evidence_hash is not canonical lowercase sha256: {requirement_id}")
    if not isinstance(item.get("source_context"), dict):
        errors.append(f"insurer partner authority source_context is required: {requirement_id}")
    elif item.get("source_context") != source_context:
        errors.append(f"insurer partner authority source_context does not match source bindings: {requirement_id}")
...
def _authority_evidence_source_context(
    ...
    return {
        ...
        "partner_api_endpoint": service_binding.get("partner_api_endpoint"),
        "underwriter": service_binding.get("underwriter"),
        "quote_id": service_binding.get("quote_id"),
        "quote_ref": service_binding.get("quote_ref"),
        "telemetry_hash": service_binding.get("telemetry_hash"),
        "consent_id": service_binding.get("consent_id"),
        "risk_tier": service_binding.get("risk_tier"),
        "frontend_bundle_hash": service_binding.get("frontend_bundle_hash"),
        "actuarial_product_id": service_binding.get("actuarial_product_id"),
        ...
        "worker_policy_binding_hashes": _sorted_present(binding.get("policy_binding_hash") for binding in valid_workers),
...
def _controls(mode: str, service_binding: dict[str, Any], worker_bindings: list[Any], worker_bundle_bindings: list[dict[str, Any]], evidence: list[dict[str, Any]], summary: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        {"name": "worker_receipts_bound", "status": "passed" if service_binding.get("attestation_id") and worker_bindings else "failed", "detail": "The dossier binds insurer partner worker operation IDs, service hashes, scheduler, queue, delivery, policy, response, log, and credential refs."},
        {"name": "worker_review_bundles_bound", "status": "passed" if worker_bundle_bindings else "deferred", "detail": "Optional worker review bundles bind offline source-byte replay, frontend bundle replay, actuarial corpus replay, telemetry, underwriting quote, and policy binding evidence to the authority dossier."},
