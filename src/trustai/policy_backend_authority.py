    ...
    *,
    ...
    if item.get("evidence_hash"):
        try:
            ...
            errors.append(f"invalid policy backend authority evidence bundle: {exc}")
            return "missing"
        if item.get("evidence_hash") != normalized_hash:
            errors.append(f"policy backend authority evidence bundle evidence_hash is not canonical lowercase sha256: {requirement_id}")
    ...
    *,
    ...
    if item.get("evidence_hash"):
        try:
            ...
            errors.append(f"invalid policy backend authority evidence: {exc}")
            return "missing"
        if item.get("evidence_hash") != normalized_hash:
            errors.append(f"policy backend authority evidence_hash is not canonical lowercase sha256: {requirement_id}")
