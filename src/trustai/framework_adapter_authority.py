...
def _normalize_sha256_ref(value: Any, field: str) -> str:
    ...
    if not value.startswith("sha256:"):
        raise ValueError(f"framework adapter authority {field} must start with sha256:")
    ...
    if len(hexdigest) != 64 or any(character not in "0123456789abcdefABCDEF" for character in hexdigest):
        raise ValueError(f"framework adapter authority {field} must contain a 64-character sha256 digest")
