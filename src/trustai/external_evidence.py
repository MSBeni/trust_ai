...
@dataclass
class ExternalEvidenceProductionReplacementSubmissionVerification:
    ok: bool
    errors: list[str]
    warnings: list[str]
...
def _external_evidence_production_replacement_request_refs(request: dict[str, Any]) -> set[str]:
    refs = _source_map_entry_refs(request)
    fulfillment = request.get("fulfillment") if isinstance(request.get("fulfillment"), dict) else {}
    refs.update(_source_map_entry_refs(fulfillment))
    for key in ("collection_task_ref", "task_ref", "unit_ref", "task_id", "unit_id"):
...
def _external_evidence_production_replacement_merge_submission_fulfillments(
    template: dict[str, Any],
    fulfillments: list[dict[str, Any]],
) -> tuple[dict[str, Any], list[str]]:
    if template.get("schema") != EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_INTAKE_TEMPLATE_SCHEMA:
        raise ValueError(f"unsupported external evidence production replacement intake template schema: {template.get('schema')}")
    if template.get("production_replacement_intake_template_id") != content_hash(without_keys(template, "production_replacement_intake_template_id")):
        raise ValueError("production_replacement_intake_template_id does not match canonical production replacement intake template body")
    if not fulfillments:
        raise ValueError("at least one production replacement fulfillment is required")
    body = json.loads(json.dumps(without_keys(template, "production_replacement_intake_template_id"), sort_keys=True))
    rows = body.get("fulfillments")
    if not isinstance(rows, list):
        raise ValueError("production replacement intake template fulfillments must be a list")
    requests = body.get("requests")
    if not isinstance(requests, list):
        raise ValueError("production replacement intake template requests must be a list")

    submitted_refs: set[str] = set()
    for index, fulfillment in enumerate(fulfillments):
        if not isinstance(fulfillment, dict):
            raise ValueError(f"production replacement fulfillment {index} must be an object")
        task_ref = _source_map_fulfillment_task_ref(fulfillment)
        if not task_ref:
            raise ValueError(f"production replacement fulfillment {index} is missing task reference")
        matches = [row for row in rows if isinstance(row, dict) and task_ref in _source_map_entry_refs(row)]
        if not matches:
            raise ValueError(f"production replacement fulfillment task not found: {task_ref}")
        if len(matches) > 1:
            raise ValueError(f"production replacement fulfillment task is ambiguous: {task_ref}")
        row = matches[0]
        canonical_ref = str(row.get("task") or row.get("unit_ref") or task_ref)
        if canonical_ref in submitted_refs:
            raise ValueError(f"duplicate production replacement fulfillment for task: {canonical_ref}")
        submitted_refs.add(canonical_ref)
        for key, value in fulfillment.items():
            if key in SOURCE_MAP_FULFILLMENT_TASK_KEYS:
                continue
            if key not in SOURCE_MAP_FULFILLMENT_FIELDS:
                raise ValueError(f"unsupported production replacement fulfillment key: {key}")
            if key == "timeout_seconds":
                try:
                    timeout = float(value)
                except (TypeError, ValueError) as exc:
                    raise ValueError("production replacement fulfillment timeout_seconds must be numeric") from exc
                if timeout <= 0:
                    raise ValueError("production replacement fulfillment timeout_seconds must be positive")
                row[key] = timeout
            else:
                text_value = str(value or "").strip()
                if key in {"source_uri", "description"} and not text_value:
                    raise ValueError(f"production replacement fulfillment {key} is required")
                row[key] = text_value
        row_refs = _source_map_entry_refs(row)
        for request in requests:
            if isinstance(request, dict) and row_refs.intersection(_external_evidence_production_replacement_request_refs(request)):
                request["fulfillment"] = json.loads(json.dumps(row, sort_keys=True))

    summary = body.get("summary")
    if not isinstance(summary, dict):
        raise ValueError("production replacement intake template summary must be an object")
    source_counts = _source_map_source_uri_counts([row for row in rows if isinstance(row, dict)])
    submitted_rows = [row for row in rows if isinstance(row, dict) and str(row.get("task") or row.get("unit_ref") or "") in submitted_refs]
    submitted_counts = _source_map_source_uri_counts(submitted_rows)
    summary["request_count"] = len(requests)
    summary["fulfillment_count"] = len(rows)
    summary["submitted_task_count"] = len(submitted_refs)
    summary["submitted_placeholder_source_uri_count"] = submitted_counts.get("placeholder_source_uri_count", 0)
    summary["submitted_live_source_uri_count"] = submitted_counts.get("live_source_uri_count", 0)
    summary.update(source_counts)
    body["summary"] = summary
    submitted_template = {**body, "production_replacement_intake_template_id": content_hash(body)}
    return submitted_template, sorted(submitted_refs)


def build_external_evidence_production_replacement_submission(
    template: dict[str, Any],
    fulfillments: list[dict[str, Any]],
    *,
    generated_at: str | None = None,
) -> dict[str, Any]:
    submitted_template, submitted_refs = _external_evidence_production_replacement_merge_submission_fulfillments(
        template,
        fulfillments,
    )
    summary = submitted_template.get("summary", {}) if isinstance(submitted_template.get("summary"), dict) else {}
    body = {
        "schema": EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_SUBMISSION_SCHEMA,
        "generated_at": generated_at or utc_now(),
        "source_intake_template": {
            "production_replacement_intake_template_id": template.get("production_replacement_intake_template_id"),
            "production_replacement_intake_template_hash": content_hash(template),
            "schema": template.get("schema"),
            "generated_at": template.get("generated_at"),
        },
        "summary": {
            "submission_status": "blocked" if int(summary.get("submitted_placeholder_source_uri_count") or 0) else "submitted",
            "request_count": int(summary.get("request_count") or 0),
            "fulfillment_count": int(summary.get("fulfillment_count") or 0),
            "submitted_task_count": int(summary.get("submitted_task_count") or 0),
            "submitted_placeholder_source_uri_count": int(summary.get("submitted_placeholder_source_uri_count") or 0),
            "submitted_live_source_uri_count": int(summary.get("submitted_live_source_uri_count") or 0),
            ...
            "owner_count": int(summary.get("owner_count") or 0),
            "open_task_count": int(summary.get("open_task_count") or 0),
            "blocked_task_count": int(summary.get("blocked_task_count") or 0),
            "closed_task_count": int(summary.get("closed_task_count") or 0),
        },
        "submitted_task_refs": submitted_refs,
        "submitted_fulfillments": json.loads(json.dumps(fulfillments, sort_keys=True)),
    ...
    return {**body, "production_replacement_submission_id": content_hash(body)}


def verify_external_evidence_production_replacement_submission(
    submission: dict[str, Any],
    template: dict[str, Any],
    *,
    require_submitted_live_source_uris: bool = False,
) -> ExternalEvidenceProductionReplacementSubmissionVerification:
    errors: list[str] = []
    warnings: list[str] = []
    if submission.get("schema") != EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_SUBMISSION_SCHEMA:
        errors.append(f"unsupported external evidence production replacement submission schema: {submission.get('schema')}")
    if submission.get("production_replacement_submission_id") != content_hash(without_keys(submission, "production_replacement_submission_id")):
        errors.append("production_replacement_submission_id does not match canonical production replacement submission body")
    source = submission.get("source_intake_template") if isinstance(submission.get("source_intake_template"), dict) else {}
    if source.get("production_replacement_intake_template_id") != template.get("production_replacement_intake_template_id"):
        errors.append("production replacement submission source intake template id does not match supplied template")
    if source.get("production_replacement_intake_template_hash") != content_hash(template):
        errors.append("production replacement submission source intake template hash does not match supplied template")
    fulfillments = submission.get("submitted_fulfillments")
    if not isinstance(fulfillments, list):
        errors.append("production replacement submission submitted_fulfillments must be a list")
        fulfillments = []
    try:
        expected = build_external_evidence_production_replacement_submission(
            template,
            fulfillments,
            generated_at=str(submission.get("generated_at") or ""),
        )
    except ValueError as exc:
        errors.append(str(exc))
    else:
        if without_keys(submission, "production_replacement_submission_id") != without_keys(expected, "production_replacement_submission_id"):
            errors.append("production replacement submission body does not match supplied intake template and submitted fulfillments")
    summary = submission.get("summary", {}) if isinstance(submission.get("summary"), dict) else {}
    submitted_placeholder_count = int(summary.get("submitted_placeholder_source_uri_count") or 0)
    placeholder_count = int(summary.get("placeholder_source_uri_count") or 0)
    if submitted_placeholder_count:
        warnings.append(f"production replacement submission contains {submitted_placeholder_count} submitted placeholder source_uri values")
    if placeholder_count:
        warnings.append(f"production replacement submitted template contains {placeholder_count} total placeholder source_uri values")
    if require_submitted_live_source_uris and submitted_placeholder_count:
        errors.append("production replacement submitted fulfillments still contain placeholder source_uri values")
    return ExternalEvidenceProductionReplacementSubmissionVerification(ok=not errors, errors=errors, warnings=warnings)
...
def _external_evidence_production_replacement_queue_counts(records: list[dict[str, Any]], field: str, default: str = "unknown") -> dict[str, int]:
    counts: dict[str, int] = {}
    for record in records:
...
def _external_evidence_production_replacement_queue_reason_counts(records: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for record in records:
        reasons = record.get("blocking_reasons") if isinstance(record.get("blocking_reasons"), list) else []
        if not reasons:
            reasons = ["review-not-ready"]
        for reason in reasons:
            value = str(reason or "review-not-ready")
            counts[value] = counts.get(value, 0) + 1
    return dict(sorted(counts.items()))


def _external_evidence_production_replacement_remediation_action(task: dict[str, Any]) -> str:
    reasons = task.get("blocking_reasons") if isinstance(task.get("blocking_reasons"), list) else []
    if "placeholder-source-uri" in reasons or task.get("source_uri_status") == "placeholder":
        return "replace source_uri and authority metadata with a live production authority-owned source, then regenerate the submission review"
    return "resolve the source-map verification blocker, then regenerate the submission review"


def _external_evidence_production_replacement_remediation_item(task: dict[str, Any]) -> dict[str, Any]:
    task_ref = str(task.get("task") or task.get("unit_ref") or task.get("task_ref") or "").strip()
    requirement_id = str(task.get("requirement_id") or "").strip()
    authority_kind = str(task.get("authority_kind") or "").strip()
    owner_hint = str(task.get("owner_hint") or "unassigned").strip() or "unassigned"
    source_uri = str(task.get("source_uri") or "").strip()
    blocking_reasons = list(task.get("blocking_reasons") or [])
    if task.get("review_status") != "ready-to-collect" and not blocking_reasons:
        blocking_reasons.append("review-not-ready")
    return {
        "task": task_ref,
        "task_ref": task.get("task_ref"),
        "owner_hint": owner_hint,
        "requirement_id": requirement_id or task.get("requirement_id"),
        "authority_kind": authority_kind or task.get("authority_kind"),
        "source_uri": source_uri or None,
        "source_uri_status": task.get("source_uri_status") or ("placeholder" if _source_map_is_placeholder_uri(source_uri) else "live"),
        "review_status": task.get("review_status"),
        "blocking_reasons": blocking_reasons,
        "remediation_action": _external_evidence_production_replacement_remediation_action(task),
        "snapshot_out": task.get("snapshot_out"),
        "intake_out": task.get("intake_out"),
        "fulfillment_template": {
            "task": task_ref,
            "source_uri": "REPLACE_WITH_LIVE_AUTHORITY_URI",
            "description": f"Production authority evidence for {requirement_id}:{authority_kind}".strip(":"),
            "source_file": task.get("snapshot_out"),
            "retrieval_method": "authority-export",
            "content_type": "application/json",
            "issuer": "REPLACE_WITH_AUTHORITY_ISSUER",
            "subject": f"REPLACE_WITH_AUTHORITY_SUBJECT_FOR_{task_ref}" if task_ref else "REPLACE_WITH_AUTHORITY_SUBJECT",
            "issued_at": "REPLACE_WITH_RFC3339_ISSUED_AT",
            "expires_at": "REPLACE_WITH_RFC3339_EXPIRES_AT",
        },
        "replaces_artifacts": list(task.get("replaces_artifacts") or []),
        "replaces_evidence_ids": list(task.get("replaces_evidence_ids") or []),
    }
...
def build_external_evidence_production_replacement_remediation_queue(
    review: dict[str, Any],
    *,
    generated_at: str | None = None,
) -> dict[str, Any]:
    ...
    remediation_items = [
        _external_evidence_production_replacement_remediation_item(task)
        for task in task_reviews
        if isinstance(task, dict) and task.get("review_status") != "ready-to-collect"
    ]
    remediation_items.sort(
        key=lambda item: (
            str(item.get("owner_hint") or ""),
    ...
    summary = review.get("summary") if isinstance(review.get("summary"), dict) else {}
    owner_count = len({str(item.get("owner_hint") or "unassigned") for item in remediation_items})
    body = {
        "schema": EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_REMEDIATION_QUEUE_SCHEMA,
        "generated_at": generated_at or utc_now(),
        "sources": {
        ...
        "summary": {
            "queue_status": "clear" if not remediation_items else "blocked",
            ...
            "remediation_task_count": len(remediation_items),
            "blocked_task_count": len(remediation_items),
            "owner_count": owner_count,
            "placeholder_source_uri_count": sum(1 for item in remediation_items if item.get("source_uri_status") == "placeholder"),
            "live_source_uri_count": sum(1 for item in remediation_items if item.get("source_uri_status") == "live"),
            "review_placeholder_source_uri_count": int(summary.get("placeholder_source_uri_count") or 0),
            "review_live_source_uri_count": int(summary.get("live_source_uri_count") or 0),
            "remediation_count_by_owner_hint": _external_evidence_production_replacement_queue_counts(remediation_items, "owner_hint", "unassigned"),
            "remediation_count_by_authority_kind": _external_evidence_production_replacement_queue_counts(remediation_items, "authority_kind", "unknown"),
            "remediation_count_by_requirement": _external_evidence_production_replacement_queue_counts(remediation_items, "requirement_id", "unknown"),
            "remediation_count_by_source_uri_status": _external_evidence_production_replacement_queue_counts(remediation_items, "source_uri_status", "unknown"),
            "remediation_count_by_blocking_reason": _external_evidence_production_replacement_queue_reason_counts(remediation_items),
        },
    ...
    return {**body, "production_replacement_remediation_queue_id": content_hash(body)}


def verify_external_evidence_production_replacement_remediation_queue(
    queue: dict[str, Any],
    review: dict[str, Any],
    *,
    require_empty: bool = False,
) -> ExternalEvidenceProductionReplacementRemediationQueueVerification:
    errors: list[str] = []
    warnings: list[str] = []
    if queue.get("schema") != EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_REMEDIATION_QUEUE_SCHEMA:
        errors.append(f"unsupported external evidence production replacement remediation queue schema: {queue.get('schema')}")
    if queue.get("production_replacement_remediation_queue_id") != content_hash(without_keys(queue, "production_replacement_remediation_queue_id")):
        errors.append("production_replacement_remediation_queue_id does not match canonical production replacement remediation queue body")
    sources = queue.get("sources") if isinstance(queue.get("sources"), dict) else {}
    if sources.get("production_replacement_submission_review") != _external_evidence_review_source_record(
        review,
        "production_replacement_submission_review_id",
        "production_replacement_submission_review_hash",
    ):
        errors.append("production replacement remediation queue source review does not match supplied review")
    try:
        expected = build_external_evidence_production_replacement_remediation_queue(
            review,
        ...
        if without_keys(queue, "production_replacement_remediation_queue_id") != without_keys(expected, "production_replacement_remediation_queue_id"):
            errors.append("production replacement remediation queue body does not match supplied submission review")
    ...
    if remediation_count:
        warnings.append(f"production replacement remediation queue contains {remediation_count} blocked tasks")
    if require_empty and remediation_count:
        errors.append("production replacement remediation queue is not empty")
    return ExternalEvidenceProductionReplacementRemediationQueueVerification(ok=not errors, errors=errors, warnings=warnings)
...
def _verify_external_evidence_production_replacement_remediation_owner_fulfillment_submission(
    template: dict[str, Any],
    packet_bundle: dict[str, Any],
) -> ExternalEvidenceProductionReplacementRemediationOwnerFulfillmentTemplateVerification:
    errors: list[str] = []
    ...
    if template.get("production_replacement_remediation_owner_fulfillment_template_id") != content_hash(
        ...
        warnings.append("production replacement remediation owner fulfillment template id is stale after owner edits; review records the submitted content hash")
    source = template.get("source_remediation_owner_packets") if isinstance(template.get("source_remediation_owner_packets"), dict) else {}
    if source.get("production_replacement_remediation_owner_packet_bundle_id") != packet_bundle.get("production_replacement_remediation_owner_packet_bundle_id"):
        errors.append("production replacement remediation owner fulfillment template source packet bundle id does not match supplied packets")
    if source.get("production_replacement_remediation_owner_packet_bundle_hash") != content_hash(packet_bundle):
    ...
    if expected is not None:
        ...
        if without_keys(template, *stable_keys) != without_keys(expected, *stable_keys):
            errors.append("production replacement remediation owner fulfillment submission metadata does not match supplied owner packets")
        expected_requests = {
            str(item.get("task") or ""): without_keys(item, "fulfillment")
            for item in expected.get("requests", [])
            if isinstance(item, dict)
        }
        actual_requests = {
        ...
        if actual_requests != expected_requests:
            errors.append("production replacement remediation owner fulfillment request context does not match supplied owner packets")
        expected_tasks = [str(item.get("task") or "") for item in expected.get("fulfillments", []) if isinstance(item, dict)]
        actual_tasks = [str(item.get("task") or "") for item in template.get("fulfillments", []) if isinstance(item, dict)]
        if sorted(actual_tasks) != sorted(expected_tasks):
            errors.append("production replacement remediation owner fulfillment submission tasks do not match supplied owner packets")
    summary = template.get("summary", {}) if isinstance(template.get("summary"), dict) else {}
    if summary.get("placeholder_source_uri_count"):
        warnings.append(f"production replacement remediation owner fulfillment template summary contains {summary.get('placeholder_source_uri_count')} placeholder source_uri values")
    return ExternalEvidenceProductionReplacementRemediationOwnerFulfillmentTemplateVerification(ok=not errors, errors=errors, warnings=warnings)


def _external_evidence_production_replacement_remediation_owner_fulfillment_review_task_records(
    template: dict[str, Any],
    fulfilled_source_map: dict[str, Any],
) -> list[dict[str, Any]]:
    request_by_task: dict[str, dict[str, Any]] = {}
        ...
        records.append(
            {
                "task": task_ref,
                "task_ref": entry.get("task_ref"),
                "packet_ref": request.get("packet_ref"),
                ...
                "review_status": "blocked" if blocking_reasons else "ready-to-collect",
                "blocking_reasons": blocking_reasons,
                "original_blocking_reasons": request.get("blocking_reasons") if isinstance(request.get("blocking_reasons"), list) else [],
...
def _external_evidence_production_replacement_remediation_owner_fulfillment_review_blockers(
    template_result: ExternalEvidenceProductionReplacementRemediationOwnerFulfillmentTemplateVerification,
    source_map_result: ExternalEvidenceSourceMapVerification,
    placeholder_count: int,
) -> list[str]:
    blockers: list[str] = []
    if not template_result.ok:
        blockers.extend(f"production replacement remediation owner fulfillment template: {error}" for error in template_result.errors)
    if placeholder_count:
        blockers.append(f"production replacement remediation owner fulfillment review contains {placeholder_count} placeholder source_uri values")
    if not source_map_result.ok:
        blockers.extend(f"fulfilled production source map: {error}" for error in source_map_result.errors)
    return blockers


def _external_evidence_production_replacement_remediation_owner_fulfillment_review_next_actions(review_status: str, placeholder_count: int) -> list[str]:
    if placeholder_count:
        return [
    ...
    return [
        "Resolve the fulfilled source-map verification errors in this owner remediation review.",
        "Regenerate this review after the owner fulfillment template and generated source map verify cleanly.",
    ]


def build_external_evidence_production_replacement_remediation_owner_fulfillment_review(
    template: dict[str, Any],
    packet_bundle: dict[str, Any],
    plan: dict[str, Any],
    *,
    ...
    review_generated_at = generated_at or utc_now()
    template_result = _verify_external_evidence_production_replacement_remediation_owner_fulfillment_submission(template, packet_bundle)
    if not template_result.ok:
        raise ValueError(
            "external evidence production replacement remediation owner fulfillment is not valid for review: "
            + "; ".join(template_result.errors)
        )
    source_map_template = _build_external_evidence_production_replacement_source_map_template(
        plan,
        snapshot_dir=snapshot_dir,
    ...
    fulfilled_summary = fulfilled_source_map.get("summary", {}) if isinstance(fulfilled_source_map.get("summary"), dict) else {}
    template_summary = template.get("summary", {}) if isinstance(template.get("summary"), dict) else {}
    task_reviews = _external_evidence_production_replacement_remediation_owner_fulfillment_review_task_records(template, fulfilled_source_map)
    ready_task_count = sum(1 for task in task_reviews if task.get("review_status") == "ready-to-collect")
    blocked_task_count = sum(1 for task in task_reviews if task.get("review_status") == "blocked")
    placeholder_count = int(fulfilled_summary.get("placeholder_source_uri_count") or 0)
    review_status = "ready-to-collect" if source_map_result.ok and blocked_task_count == 0 and placeholder_count == 0 else "blocked"
    blockers = _external_evidence_production_replacement_remediation_owner_fulfillment_review_blockers(
        template_result,
        source_map_result,
        placeholder_count,
    )
    body = {
        "schema": EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_REMEDIATION_OWNER_FULFILLMENT_REVIEW_SCHEMA,
        "generated_at": review_generated_at,
        "verification_options": {
            "require_live_source_uris": require_live_source_uris,
            "require_source_snapshots": require_source_snapshots,
        ...
        "sources": {
            "production_replacement_remediation_owner_fulfillment_template": _external_evidence_review_source_record(
                template,
                "production_replacement_remediation_owner_fulfillment_template_id",
                "production_replacement_remediation_owner_fulfillment_template_hash",
            ),
            "production_replacement_remediation_owner_packets": _external_evidence_review_source_record(
                packet_bundle,
                "production_replacement_remediation_owner_packet_bundle_id",
                "production_replacement_remediation_owner_packet_bundle_hash",
            ),
            "collection_plan": _external_evidence_review_source_record(plan, "plan_id", "plan_hash"),
        },
        "summary": {
    ...
    return {**body, "production_replacement_remediation_owner_fulfillment_review_id": content_hash(body)}


def verify_external_evidence_production_replacement_remediation_owner_fulfillment_review(
    review: dict[str, Any],
    template: dict[str, Any],
    packet_bundle: dict[str, Any],
    plan: dict[str, Any],
    *,
    ...
    now: str | None = None,
    require_ready: bool = False,
) -> ExternalEvidenceProductionReplacementRemediationOwnerFulfillmentReviewVerification:
    errors: list[str] = []
    warnings: list[str] = []
    if review.get("schema") != EXTERNAL_EVIDENCE_PRODUCTION_REPLACEMENT_REMEDIATION_OWNER_FULFILLMENT_REVIEW_SCHEMA:
        errors.append(f"unsupported external evidence production replacement remediation owner fulfillment review schema: {review.get('schema')}")
    if review.get("production_replacement_remediation_owner_fulfillment_review_id") != content_hash(
        without_keys(review, "production_replacement_remediation_owner_fulfillment_review_id")
    ):
        errors.append("production_replacement_remediation_owner_fulfillment_review_id does not match canonical review body")
    expected_options = {
        "require_live_source_uris": require_live_source_uris,
        "require_source_snapshots": require_source_snapshots,
        "require_fresh_source_snapshots": require_fresh_source_snapshots,
    ...
    if review.get("verification_options") != expected_options:
        errors.append("verification_options do not match verifier options")
    sources = review.get("sources") if isinstance(review.get("sources"), dict) else {}
    if sources.get("production_replacement_remediation_owner_fulfillment_template") != _external_evidence_review_source_record(
        template,
        "production_replacement_remediation_owner_fulfillment_template_id",
        "production_replacement_remediation_owner_fulfillment_template_hash",
    ):
        errors.append("production replacement remediation owner fulfillment review source template does not match supplied template")
    if sources.get("production_replacement_remediation_owner_packets") != _external_evidence_review_source_record(
        packet_bundle,
        "production_replacement_remediation_owner_packet_bundle_id",
        "production_replacement_remediation_owner_packet_bundle_hash",
    ):
        errors.append("production replacement remediation owner fulfillment review source owner packets do not match supplied packets")
    if sources.get("collection_plan") != _external_evidence_review_source_record(plan, "plan_id", "plan_hash"):
        errors.append("production replacement remediation owner fulfillment review source collection plan does not match supplied plan")
    try:
        expected = build_external_evidence_production_replacement_remediation_owner_fulfillment_review(
            template,
            packet_bundle,
            plan,
            root=root,
        ...
        if without_keys(review, "production_replacement_remediation_owner_fulfillment_review_id") != without_keys(
            expected,
            "production_replacement_remediation_owner_fulfillment_review_id",
        ):
            errors.append("production replacement remediation owner fulfillment review body does not match supplied template, owner packets, and collection plan")
    summary = review.get("summary", {}) if isinstance(review.get("summary"), dict) else {}
    verification = review.get("verification", {}) if isinstance(review.get("verification"), dict) else {}
    warnings.extend(verification.get("template_warnings", []) if isinstance(verification.get("template_warnings"), list) else [])
    warnings.extend(verification.get("fulfilled_source_map_warnings", []) if isinstance(verification.get("fulfilled_source_map_warnings"), list) else [])
    if summary.get("placeholder_source_uri_count"):
        warnings.append(
            "production replacement remediation owner fulfillment review contains "
            f"{summary.get('placeholder_source_uri_count')} placeholder source_uri values"
        )
    if require_ready and summary.get("review_status") != "ready-to-collect":
        errors.append("production replacement remediation owner fulfillment review is not ready to collect")
...
def write_external_evidence_production_replacement_submission(path: str | Path, submission: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(submission, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def load_external_evidence_production_replacement_submission(path: str | Path) -> dict[str, Any]:
    return json.loads(Path(path).read_text(encoding="utf-8"))


def write_external_evidence_production_replacement_submission_markdown(path: str | Path, submission: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(render_external_evidence_production_replacement_submission_markdown(submission), encoding="utf-8")
...
def write_external_evidence_production_replacement_remediation_queue(path: str | Path, queue: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(queue, indent=2, sort_keys=True), encoding="utf-8")


def load_external_evidence_production_replacement_remediation_queue(path: str | Path) -> dict[str, Any]:
    return json.loads(Path(path).read_text(encoding="utf-8"))


def write_external_evidence_production_replacement_remediation_queue_markdown(path: str | Path, queue: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
...
def write_external_evidence_production_replacement_remediation_owner_fulfillment_review(path: str | Path, review: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(review, indent=2, sort_keys=True), encoding="utf-8")


def load_external_evidence_production_replacement_remediation_owner_fulfillment_review(path: str | Path) -> dict[str, Any]:
    return json.loads(Path(path).read_text(encoding="utf-8"))


def write_external_evidence_production_replacement_remediation_owner_fulfillment_review_markdown(path: str | Path, review: dict[str, Any]) -> None:
    target = Path(path)
    target.parent.mkdir(parents=True, exist_ok=True)
...
def render_external_evidence_production_replacement_submission_markdown(submission: dict[str, Any]) -> str:
    summary = submission.get("summary", {}) if isinstance(submission.get("summary"), dict) else {}
    lines = [
        "# External Evidence Production Replacement Submission",
        "",
        f"- Submission ID: `{submission.get('production_replacement_submission_id')}`",
        f"- Generated at: `{submission.get('generated_at')}`",
        f"- Submitted tasks: {summary.get('submitted_task_count', 0)}",
        f"- Submitted live source URIs: {summary.get('submitted_live_source_uri_count', 0)}",
        f"- Submitted placeholder source URIs: {summary.get('submitted_placeholder_source_uri_count', 0)}",
        f"- Total placeholder source URIs after submission: {summary.get('placeholder_source_uri_count', 0)}",
        f"- Total live source URIs after submission: {summary.get('live_source_uri_count', 0)}",
        "",
    ...
    if isinstance(fulfillments, list):
        for fulfillment in fulfillments:
            if not isinstance(fulfillment, dict):
                continue
            lines.append(
                f"| `{_markdown_cell(_source_map_fulfillment_task_ref(fulfillment))}` | "
                f"{_markdown_cell(fulfillment.get('source_uri'))} | "
                f"{_markdown_cell(fulfillment.get('description'))} | "
                f"`{_markdown_cell(fulfillment.get('source_file'))}` |"
            )
    commands = submission.get("commands", {}) if isinstance(submission.get("commands"), dict) else {}
    lines.extend(["", "## Commands", ""])
    if commands:
        for key, command in commands.items():
            lines.append(f"- {key}: `{_markdown_cell(command)}`")
...
def render_external_evidence_production_replacement_remediation_owner_fulfillment_review_markdown(review: dict[str, Any]) -> str:
    summary = review.get("summary", {}) if isinstance(review.get("summary"), dict) else {}
    lines = [
        "# External Evidence Production Replacement Remediation Owner Fulfillment Review",
        "",
        f"- Review ID: `{review.get('production_replacement_remediation_owner_fulfillment_review_id')}`",
        f"- Generated at: `{review.get('generated_at')}`",
        f"- Status: `{summary.get('review_status')}`",
        f"- Requests: {summary.get('request_count', 0)}",
        f"- Fulfillments: {summary.get('fulfillment_count', 0)}",
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        ...
                        _markdown_cell(task.get("snapshot_out")),
                        _markdown_cell(task.get("intake_out")),
                        _markdown_cell(task.get("remediation_action")),
                    ]
                )
                + " |"
            )
...
def render_external_evidence_production_replacement_remediation_queue_markdown(queue: dict[str, Any]) -> str:
    summary = queue.get("summary", {}) if isinstance(queue.get("summary"), dict) else {}
    lines = [
        "# External Evidence Production Replacement Remediation Queue",
        "",
        f"- Queue ID: `{queue.get('production_replacement_remediation_queue_id')}`",
        f"- Generated at: `{queue.get('generated_at')}`",
        f"- Status: `{summary.get('queue_status')}`",
        ...
        f"- Owners: {summary.get('owner_count', 0)}",
        f"- Placeholder source URIs: {summary.get('placeholder_source_uri_count', 0)}",
        f"- Live source URIs in blocked rows: {summary.get('live_source_uri_count', 0)}",
        "",
        "## Counts By Owner",
        "",
    ]
    owner_counts = summary.get("remediation_count_by_owner_hint", {}) if isinstance(summary.get("remediation_count_by_owner_hint"), dict) else {}
    if owner_counts:
        lines.extend(f"- {_markdown_cell(owner)}: {count}" for owner, count in owner_counts.items())
    else:
        lines.append("- None")
    lines.extend(["", "## Counts By Authority", ""])
    authority_counts = summary.get("remediation_count_by_authority_kind", {}) if isinstance(summary.get("remediation_count_by_authority_kind"), dict) else {}
    if authority_counts:
        lines.extend(f"- `{_markdown_cell(authority)}`: {count}" for authority, count in authority_counts.items())
    else:
        lines.append("- None")
    lines.extend([
        "",
    ...
    items = queue.get("remediation_items", []) if isinstance(queue.get("remediation_items"), list) else []
    if items:
        for item in items:
            if not isinstance(item, dict):
                continue
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        ...
                        _markdown_cell(item.get("remediation_action")),
                    ]
                )
                + " |"
            )
        ...
        lines.append("| - | - | - | - | - | - | - |")
    blockers = queue.get("review_blockers", []) if isinstance(queue.get("review_blockers"), list) else []
    lines.extend(["", "## Review Blockers", ""])
    if blockers:
        lines.extend(f"- {_markdown_cell(blocker)}" for blocker in blockers)
    else:
        lines.append("- None")
    commands = queue.get("commands", {}) if isinstance(queue.get("commands"), dict) else {}
    lines.extend(["", "## Commands", ""])
    if commands:
        for key, command in commands.items():
            lines.append(f"- {key}: `{_markdown_cell(command)}`")
        ...
        lines.append("- None")
    next_actions = queue.get("next_actions", []) if isinstance(queue.get("next_actions"), list) else []
    lines.extend(["", "## Next Actions", ""])
    if next_actions:
        lines.extend(f"- {_markdown_cell(action)}" for action in next_actions)
    else:
