...
CONFORMANCE_TARGETS = (
    ...
    },
    {
        ...
        "description": "Traffic holdout export and completeness receipts bind production traffic source refs, extraction windows, replay record hashes, collector/provider stream records, cursor bounds, audit records, freeze and holdout boundaries, privacy limits, and chain append evidence before shadow replay promotion evidence is trusted.",
    ...
    },
    {
        ...
        "description": "Provider delivery service attestations bind provider delivery receipts to dispatch worker fleet, queue, DLQ, retry, idempotency, outbound proxy, provider endpoint, egress, rate limits, request signing, audit, metrics, actor, redacted credentials, and optional provider operations service evidence.",
    ...
    },
    {
        ...
        "description": "Provider delivery worker receipts bind individual dispatch worker runs to delivery service attestations, source delivery receipts, scheduler leases, checkpoints, queue/DLQ metadata, idempotency, provider request/response hashes, delivery/provider/audit log roots, and redacted worker/provider credentials.",
