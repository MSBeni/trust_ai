...
from .review_portal_service import verify_review_portal_service_attestation
...
PRODUCTION_AUTHORITY_REQUIREMENTS = [
    {"id": "hosted-portal-worker-fleet", "title": "Continuously operated hosted review portal workers and API service", "authority_kinds": ["hosted-service", "provider-api"]},
    {"id": "production-identity-provider-sessions", "title": "Production identity-provider sessions, MFA, and reviewer authentication events", "authority_kinds": ["identity-provider", "provider-api"]},
    {"id": "regulator-auditor-account-lifecycle", "title": "Regulator and auditor account provisioning, revocation, and lifecycle operation evidence", "authority_kinds": ["identity-provider", "provider-api", "customer"]},
    {"id": "immutable-access-logs", "title": "Immutable portal access logs, session logs, and reviewer activity retention", "authority_kinds": ["cloud-object-lock", "provider-api", "hosted-service"]},
    {"id": "hosted-frontend-bundle-release", "title": "Hosted frontend bundle release, CDN/WAF/TLS, and source integrity evidence", "authority_kinds": ["hosted-service", "provider-api", "ci-run"]},
    {"id": "selective-disclosure-enforcement", "title": "Selective disclosure enforcement, scoped Merkle proof access, and data minimization controls", "authority_kinds": ["hosted-service", "customer", "provider-api"]},
    {"id": "tenant-isolation-rbac", "title": "Tenant isolation, RBAC policy, session policy, and authorization enforcement", "authority_kinds": ["hosted-service", "identity-provider", "provider-api"]},
    {"id": "kms-session-data-encryption", "title": "KMS-backed session store and disclosed artifact encryption evidence", "authority_kinds": ["kms-hsm", "provider-api"]},
    {"id": "portal-observability-alerting", "title": "Portal service metrics, alert policy, audit-log root, and incident response evidence", "authority_kinds": ["hosted-service", "provider-api"]},
    {"id": "supervised-access-session-replay", "title": "Replayable supervised-access session exports linked to accepted proof packs and disclosures", "authority_kinds": ["hosted-service", "regulator", "customer"]},
]
...
def load_review_portal_authority_dossier(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("review portal authority dossier must contain an object")
    return value


def write_review_portal_authority_dossier(path: str | Path, dossier: dict[str, Any]) -> None:
    target = Path(path)
...
def build_review_portal_authority_dossier(
    ...
    *,
    ...
    if mode not in REVIEW_PORTAL_AUTHORITY_MODES:
        raise ValueError(f"mode must be one of {sorted(REVIEW_PORTAL_AUTHORITY_MODES)}")
    for value, field in ((dossier_ref, "dossier_ref"), (authority_ref, "authority_ref"), (producer_ref, "producer_ref")):
        _require_text(value, field)
    timestamp = generated_at or utc_now()
    parse_rfc3339(timestamp)

    source_result = verify_review_portal_service_attestation(
        service_attestation,
    ...
    body: dict[str, Any] = {
        ...
        "limitations": [
            ...
            "It does not claim continuously operated production review portal authority unless mode is production-dossier and every required authority category has fresh external evidence.",
        ],
    }
    dossier_id = content_hash(body)
    return {**body, "dossier_id": dossier_id, "signatures": [sign_value({"dossier_id": dossier_id, "review_portal_authority": body}, key)]}


def verify_review_portal_authority_dossier(
    ...
    *,
    ...
    now: str | None = None,
) -> ReviewPortalAuthorityVerification:
    errors: list[str] = []
    warnings: list[str] = []
    freshness_now = _freshness_reference(dossier, now, errors)

    if dossier.get("schema") != REVIEW_PORTAL_AUTHORITY_SCHEMA:
        errors.append(f"unsupported review portal authority schema: {dossier.get('schema')}")
    body = without_keys(dossier, "dossier_id", "signatures")
    if dossier.get("dossier_id") != content_hash(body):
        errors.append("dossier_id does not match canonical review portal authority body")
    signatures = dossier.get("signatures", [])
    if not isinstance(signatures, list) or not signatures:
        errors.append("review portal authority dossier must include at least one signature")
    else:
        signed_value = {"dossier_id": dossier.get("dossier_id"), "review_portal_authority": body}
        if not any(isinstance(signature, dict) and verify_value(signed_value, signature, key) for signature in signatures):
            errors.append("review portal authority signature verification failed")

    mode = dossier.get("mode")
    if mode not in REVIEW_PORTAL_AUTHORITY_MODES:
        errors.append("review portal authority mode is unsupported")
    elif mode != "production-dossier":
        warnings.append(f"review portal authority mode is {mode}; live production portal authority is not claimed")
    try:
        parse_rfc3339(str(dossier.get("generated_at") or ""))
    except ValueError as exc:
        errors.append(f"review portal authority generated_at invalid: {exc}")
    for field in ("environment", "dossier_ref", "authority_ref", "producer_ref"):
        if not dossier.get(field):
            errors.append(f"review portal authority {field} is required")
    ...
    if dossier.get("summary") != expected_summary:
        errors.append("review portal authority summary does not match authority evidence")
    missing = expected_summary["missing_requirement_ids"]
    if missing:
        warnings.append("review portal authority evidence missing for: " + ", ".join(missing))
...
def append_review_portal_authority_dossier(
    chain: EvidenceChain,
    dossier: dict[str, Any],
    *,
    service_attestation: dict[str, Any],
    ...
    **sources: Any,
    ...
    if not result.ok:
        raise ValueError("invalid review portal authority dossier: " + "; ".join(result.errors))
    payload = {
    ...
    return chain.append(REVIEW_PORTAL_AUTHORITY_ENTRY_TYPE, payload, key=key, timestamp=dossier.get("generated_at"))
...
def _service_attestation_binding(attestation: dict[str, Any]) -> dict[str, Any]:
    ...
    access = attestation.get("access", {}) if isinstance(attestation.get("access"), dict) else {}
    ...
    return {
        ...
        "supervised_access_receipt_id": access.get("supervised_access_receipt_id"),
        "session_id": access.get("session_id"),
        "audience_type": access.get("audience_type"),
        "audience_purpose": access.get("audience_purpose"),
        "reviewer_subject_ref": access.get("reviewer_subject_ref"),
        "reviewer_organization": access.get("reviewer_organization"),
        "reviewer_role": access.get("reviewer_role"),
        "artifact_count": access.get("artifact_count"),
        ...
        "session_policy_ref": security.get("session_policy_ref"),
        "selective_disclosure_policy_ref": security.get("selective_disclosure_policy_ref"),
        ...
        "content_security_policy_ref": security.get("content_security_policy_ref"),
...
def _verify_service_attestation_binding(binding: Any, service_attestation: dict[str, Any] | None, sources: dict[str, Any], key: str | None, errors: list[str], warnings: list[str]) -> None:
    ...
    if service_attestation is None:
        warnings.append("review portal authority service attestation was not supplied; review portal service source was not replayed")
        return
    expected = _service_attestation_binding(service_attestation)
    if binding != expected:
        errors.append("review portal authority service_attestation_binding does not match supplied service attestation")
    result = verify_review_portal_service_attestation(service_attestation, key=key, **sources)
    if not result.ok:
        errors.extend(f"review portal authority service source: {error}" for error in result.errors)
...
def _verify_authority_evidence_item(
    ...
    *,
    ...
    if missing_fields:
        ...
        _freshness_problem(
            f"review portal authority freshness metadata missing for {requirement_id}: {', '.join(missing_fields)}",
            errors,
            warnings,
            require_fresh=require_fresh,
        )
    if issued_at is not None and expires_at is not None and expires_at <= issued_at:
        ...
        errors.append(f"review portal authority expires_at must be after issued_at: {requirement_id}")
    if now is not None and issued_at is not None and issued_at > now:
        freshness_status = "stale"
        _freshness_problem(
            f"review portal authority evidence is not yet issued for {requirement_id}: {item.get('issued_at')}",
            errors,
            warnings,
            require_fresh=require_fresh,
        )
    if now is not None and expires_at is not None and expires_at <= now:
        freshness_status = "stale"
        _freshness_problem(
            f"review portal authority evidence expired for {requirement_id}: {item.get('expires_at')}",
            errors,
            warnings,
            require_fresh=require_fresh,
        )
...
def _verify_required_authority(value: Any, errors: list[str]) -> None:
    if value != PRODUCTION_AUTHORITY_REQUIREMENTS:
        errors.append("review portal authority required_production_authority does not match v0.1 requirements")
    ...
    return [
        {
            ...
            "status": "passed",
            "detail": "The dossier builder replayed the review portal service attestation and supplied source evidence when available.",
        },
        {
        ...
        },
        {
            ...
            "detail": "External hosted review portal authority evidence references are hash-bound when supplied.",
        },
        {
        ...
        },
        {
            ...
            "detail": "Every hosted review portal production authority requirement must be covered before this can claim live production authority.",
        },
        {
...
def _freshness_reference(dossier: dict[str, Any], now: str | None, errors: list[str]) -> Any:
    ...
    try:
        ...
        errors.append(f"review portal authority freshness {label} invalid: {exc}")
        return None
...
def _parse_optional_timestamp(item: dict[str, Any], field: str, errors: list[str]) -> Any:
    ...
    try:
        ...
        errors.append(f"review portal authority {field} invalid: {exc}")
        return None
...
def _require_text(value: Any, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"review portal authority {field} is required")
    return value
        ...
        for key, child in value.items():
            ...
            if any(marker in normalized for marker in SECRET_KEY_MARKERS):
                if _allowed_secret_reference_key(normalized, child):
                    ...
                    errors.append(f"review portal authority contains secret-like field {child_path}; store only redacted refs")
