...
def build_provider_delivery_authority_dossier(
    ...
    *,
    ...
    service_result = verify_provider_delivery_service_attestation(service_attestation, delivery=delivery, payload=payload, provider_operations_service=provider_operations_service, key=key)
    if not service_result.ok:
        raise ValueError("invalid provider delivery service source: " + "; ".join(service_result.errors))
    for receipt in receipts:
        worker_result = verify_provider_delivery_worker_receipt(receipt, service_attestation=service_attestation, delivery=delivery, payload=payload, provider_operations_service=provider_operations_service, provider_response=provider_response, provider_audit_correlation=provider_audit_correlation, provider_audit_log=provider_audit_log, key=key)
...
def verify_provider_delivery_authority_dossier(
    ...
    *,
    ...
    _verify_service_attestation_binding(
        ...
        service_attestation,
        {"delivery": delivery, "payload": payload, "provider_operations_service": provider_operations_service},
        key,
        errors,
        warnings,
    )
...
def _verify_service_attestation_binding(binding: Any, service_attestation: dict[str, Any] | None, sources: dict[str, Any], key: str | None, errors: list[str], warnings: list[str]) -> None:
    if not isinstance(binding, dict):
        ...
        return
    for field in ("attestation_id", "attestation_hash", "attestation_mode", "environment", "attested_at", "source_count", "source_hash", "service_ref", "provider", "service_image_digest", "service_binary_hash", "replicas_min", "replicas_max", "availability_zones", "dispatch_worker_ref", "queue_ref", "dead_letter_queue_ref", "idempotency_store_ref", "retry_policy_ref", "outbound_proxy_ref", "provider_endpoint_base", "provider_credential_ref", "source_delivery_id", "source_delivery_mode", "source_target_url", "network_policy_ref", "egress_policy_ref", "rate_limit_policy_ref", "request_signing_policy_ref", "audit_log_root", "retention_until", "actor_ref", "credential_ref"):
...
def _verify_worker_receipt_bindings(bindings: Any, worker_receipts: list[dict[str, Any]] | None, service_attestation: dict[str, Any] | None, sources: dict[str, Any], key: str | None, errors: list[str], warnings: list[str]) -> None:
    ...
    for binding in bindings:
        if not isinstance(binding, dict):
            ...
            continue
        for field in ("worker_operation_id", "worker_operation_hash", "receipt_mode", "environment", "recorded_at", "source_count", "source_hash", "service_attestation_id", "service_attestation_hash", "service_ref", "provider", "worker_ref", "run_ref", "operation_kind", "actor_ref", "schedule_ref", "lease_ref", "checkpoint_ref", "checkpoint_hash", "queue_ref", "dead_letter_queue_ref", "destination_ref", "idempotency_record_hash", "provider_request_ref", "request_hash", "delivery_log_root", "provider_event_log_root", "audit_log_root", "retention_until", "credential_ref", "provider_credential_ref", "source_delivery_id", "source_delivery_hash", "source_delivery_mode", "source_provider", "target_url"):
            if binding.get(field) in (None, "", []):
    ...
    for receipt in worker_receipts:
        result = verify_provider_delivery_worker_receipt(receipt, service_attestation=service_attestation, delivery=sources.get("delivery"), payload=sources.get("payload"), provider_operations_service=sources.get("provider_operations_service"), provider_response=sources.get("provider_response"), provider_audit_correlation=sources.get("provider_audit_correlation"), provider_audit_log=sources.get("provider_audit_log"), key=key)
        if not result.ok:
