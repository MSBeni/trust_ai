...
class ExternalEvidenceManifestTests(unittest.TestCase):
    ...
    def test_retained_external_evidence_examples_verify(self):
        ...
        self.assertEqual(43, rebuilt["summary"]["covered_authority_kind_count"])
        self.assertEqual(28, rebuilt["summary"]["missing_authority_kind_count"])
        self.assertEqual(retained_manifest["summary"], rebuilt["summary"])
        self.assertEqual(28, remaining_plan["summary"]["selected_task_count"])
        self.assertEqual(28, remaining_plan["summary"]["selected_missing_task_count"])
        self.assertEqual(EXTERNAL_EVIDENCE_SOURCE_MAP_SCHEMA, source_map["schema"])
        self.assertEqual(content_hash(without_keys(source_map, "source_map_id")), source_map["source_map_id"])
        self.assertEqual(28, source_map["summary"]["entry_count"])
        self.assertEqual(28, source_map["summary"]["placeholder_source_uri_count"])
        self.assertEqual(0, source_map["summary"]["live_source_uri_count"])
        ...
        self.assertEqual(43, collected_source_map["summary"]["entry_count"])
        self.assertEqual(0, collected_source_map["summary"]["placeholder_source_uri_count"])
        self.assertEqual(43, collected_source_map["summary"]["live_source_uri_count"])
        ...
        self.assertEqual(43, collection_run["summary"]["collected_count"])
        self.assertEqual(43, collection_run_result.collected_count)
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(43, collection_payload["collected_count"])
            self.assertEqual(43, collection_payload["task_count"])
    ...
    def test_cli_external_evidence_gap_report_verifies_retained_worklist(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(43, report["summary"]["covered_authority_kind_count"])
            self.assertEqual(28, report["summary"]["missing_authority_kind_count"])
            self.assertEqual(28, report["summary"]["remaining_task_count"])
            self.assertEqual(28, report["summary"]["source_map_entry_count"])
            self.assertEqual(28, report["summary"]["placeholder_source_uri_count"])
            self.assertEqual(0, report["summary"]["live_source_uri_count"])
            first_gap = report["gaps"][0]
            self.assertEqual("compliance-mapper-and-eu-ai-act:standards-body", first_gap["unit_ref"])
            self.assertEqual("standards-body evidence for compliance-mapper-and-eu-ai-act", first_gap["description"])
            self.assertNotIn("source_file", first_gap)
            self.assertEqual("artifacts/external-evidence-sources/compliance-mapper-and-eu-ai-act/standards-body.json", first_gap["snapshot_out"])
            self.assertEqual("artifacts/external-evidence-intakes/compliance-mapper-and-eu-ai-act/standards-body.json", first_gap["intake_out"])
            markdown = markdown_path.read_text(encoding="utf-8")
            self.assertIn("# External Evidence Gap Report", markdown)
            self.assertIn("Placeholder source URIs: 28", markdown)
            self.assertIn("- Description: standards-body evidence for compliance-mapper-and-eu-ai-act", markdown)
