...
class StandardsSubmissionTests(unittest.TestCase):
    def test_standards_submission_hashes_public_specs_and_verifies(self):
        ...
        self.assertIn("shadow-replay-review-bundle-v0.1.md", markdown)
        self.assertIn("shadow-replay-review-bundle", markdown)
        ...
        self.assertIn("shadow replay review bundles", traffic_target["description"])
        self.assertTrue(any("traffic-completeness-provider-export.json" in command for command in traffic_target["commands"]))
        self.assertTrue(any("shadow-replay-review-bundle-verify" in command for command in traffic_target["commands"]))
        ...
        provider_delivery_worker_target = next(target for target in package["conformance_targets"] if target["id"] == "provider-delivery-worker-receipts")
        self.assertIn("retained payload artifact replay", provider_delivery_worker_target["description"])
        ...
        self.assertIn("retained OSS verifier CI authority export replay", verifier_release_authority_target["description"])
        self.assertTrue(any("--authority-artifact" in command for command in verifier_release_authority_target["commands"]))
        self.assertTrue(any("oss-verifier-ci-run-authority-export.json" in command for command in verifier_release_authority_target["commands"]))
