    ...
    def test_http_service_health_ingest_and_control_plane_index(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                self.assertFalse(readiness_body["production_replacement_collection_package_present"])
                self.assertFalse(readiness_body["production_replacement_collection_package_ready"])
                ...
                self.assertFalse(readiness_body["production_replacement_collection_package_summary"]["present"])
                self.assertEqual(0, readiness_body["production_replacement_worklist"]["blocked_task_total"])
                ...
                self.assertIn("no production replacement collection package indexed", readiness_body["blockers"])
                ...
                conn.request("GET", "/v0/production-replacement-worklist?limit=2")
                replacement_worklist_response = conn.getresponse()
                replacement_worklist_body = json.loads(replacement_worklist_response.read().decode("utf-8"))
                self.assertEqual(200, replacement_worklist_response.status)
                self.assertFalse(replacement_worklist_body["collection_package_present"])
                self.assertEqual([], replacement_worklist_body["blocked_tasks"])

                conn.request("GET", "/v0/control/production-replacement-worklist?limit=0")
                invalid_replacement_worklist_response = conn.getresponse()
                invalid_replacement_worklist_body = json.loads(invalid_replacement_worklist_response.read().decode("utf-8"))
                self.assertEqual(422, invalid_replacement_worklist_response.status)
                self.assertEqual("limit must be a positive integer", invalid_replacement_worklist_body["error"])
