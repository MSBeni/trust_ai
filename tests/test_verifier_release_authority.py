...
class VerifierReleaseAuthorityTests(unittest.TestCase):
    ...
    def _evidence(self, requirement_id: str = "completed-provider-workflow-run", authority_kind: str = "ci-run") -> dict:
        return {
            ...
            "evidence_hash": f"sha256:verifier-release-authority-{requirement_id}",
    ...
    def test_verifier_release_authority_cli_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            evidence = "completed-provider-workflow-run,ci-run,github-actions-run:1234567890,sha256:verifier-release-provider-workflow-run,Provider workflow run export;issuer=GitHub Actions;subject=trustai verifier release v0.1.0;source_uri=https://github.com/MSBeni/trust_ai/actions/runs/1234567890;issued_at=2026-07-16T00:10:00Z;expires_at=2026-07-23T00:10:00Z"
            ...
            subprocess.check_call([*base, "verifier-release-authority-verify", str(dossier_path), *source_args], cwd=ROOT, env=env)
            subprocess.check_call([
                *base,
