...
RETAINED_PRODUCTION_REPLACEMENT_ARTIFACTS = (
    ...
    RETAINED_PRODUCTION_REPLACEMENT_DIR / "retained-external-evidence-production-replacement-remediation-queue.json",
    RETAINED_PRODUCTION_REPLACEMENT_DIR / "retained-external-evidence-production-replacement-remediation-owner-packets.json",
    RETAINED_PRODUCTION_REPLACEMENT_DIR / "retained-external-evidence-production-replacement-remediation-owner-fulfillment-template.json",
    RETAINED_PRODUCTION_REPLACEMENT_DIR / "retained-external-evidence-production-replacement-remediation-owner-fulfillment-review.json",
    RETAINED_PRODUCTION_REPLACEMENT_DIR / "retained-external-evidence-production-replacement-remediation-application.json",
    ...
    def test_indexes_chain_and_proof_pack(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                self.assertEqual(13, summary["counts"]["external_evidence_production_replacement_lifecycle"])
                ...
                self.assertIn(
                    latest_production_replacement_lifecycle["artifact_kind"],
                    {
                        ...
                        "submission-review",
                        "remediation-queue",
                        "remediation-owner-packet-bundle",
                        "remediation-owner-fulfillment-template",
                        "remediation-owner-fulfillment-review",
                ...
                self.assertEqual(13, len(roadmap_evidence["external_evidence_production_replacement_lifecycle"]))
                ...
                self.assertTrue(readiness["production_replacement_submission_present"])
                self.assertFalse(readiness["production_replacement_submission_ready_for_review"])
                self.assertTrue(readiness["production_replacement_submission_review_present"])
                self.assertFalse(readiness["production_replacement_submission_review_ready"])
                self.assertEqual("blocked", readiness["production_replacement_submission_summary"]["submission_status"])
                self.assertEqual(72, readiness["production_replacement_submission_summary"]["submitted_task_count"])
                self.assertEqual(72, readiness["production_replacement_submission_summary"]["submitted_placeholder_source_uri_count"])
                self.assertEqual("blocked", readiness["production_replacement_submission_summary"]["review_status"])
                self.assertEqual(72, readiness["production_replacement_submission_summary"]["review_blocked_task_count"])
                self.assertEqual(72, readiness["production_replacement_submission_summary"]["review_placeholder_source_uri_count"])
                ...
                self.assertTrue(
                    any("production replacement submission is not ready for review" in blocker for blocker in readiness["blockers"])
                )
                self.assertTrue(
                    any("production replacement submission review is not ready to collect" in blocker for blocker in readiness["blockers"])
                )
                self.assertTrue(
                ...
                self.assertEqual(13, len(production_replacement_lifecycle))
                ...
                self.assertEqual("blocked", lifecycle_by_kind["remediation-queue"]["status"])
                self.assertEqual(72, lifecycle_by_kind["remediation-queue"]["blocked_task_count"])
                self.assertEqual(72, len(lifecycle_by_kind["remediation-queue"]["tasks"]))
                self.assertEqual("blocked", lifecycle_by_kind["remediation-owner-packet-bundle"]["status"])
                self.assertEqual(10, lifecycle_by_kind["remediation-owner-packet-bundle"]["packet_count"])
                self.assertEqual(10, lifecycle_by_kind["remediation-owner-packet-bundle"]["owner_count"])
                self.assertEqual(72, lifecycle_by_kind["remediation-owner-packet-bundle"]["blocked_task_count"])
                self.assertEqual("blocked", lifecycle_by_kind["remediation-owner-fulfillment-template"]["status"])
                self.assertEqual(72, lifecycle_by_kind["remediation-owner-fulfillment-template"]["request_count"])
                self.assertEqual(72, lifecycle_by_kind["remediation-owner-fulfillment-template"]["placeholder_source_uri_count"])
                self.assertEqual("blocked", lifecycle_by_kind["remediation-owner-fulfillment-review"]["status"])
                self.assertEqual(72, lifecycle_by_kind["remediation-owner-fulfillment-review"]["blocked_task_count"])
                self.assertEqual(72, lifecycle_by_kind["remediation-owner-fulfillment-review"]["placeholder_source_uri_count"])
                self.assertEqual("blocked", lifecycle_by_kind["remediation-application"]["status"])
                self.assertEqual("blocked", lifecycle_by_kind["remediation-application"]["summary"]["applied_review_status"])
                self.assertEqual(72, lifecycle_by_kind["remediation-application"]["summary"]["applied_task_count"])
                self.assertEqual(72, lifecycle_by_kind["remediation-application"]["blocked_task_count"])
                self.assertEqual(72, lifecycle_by_kind["remediation-application"]["placeholder_source_uri_count"])
                ...
                self.assertEqual("submission", production_replacement_worklist["current_stage"])
                self.assertIn("replace submitted production replacement", production_replacement_worklist["next_required_action"])
                self.assertEqual(
                    ["submission", "submission-review", "remediation-owner-fulfillment-review", "remediation-application", "collection-package", "closure"],
                    [stage["stage"] for stage in production_replacement_worklist["stage_statuses"]],
                )
                self.assertEqual(72, production_replacement_worklist["stage_statuses"][0]["blocked_task_count"])
                self.assertFalse(production_replacement_worklist["stage_statuses"][0]["ready"])
                self.assertTrue(production_replacement_worklist["submission_present"])
                self.assertFalse(production_replacement_worklist["submission_ready_for_review"])
                self.assertEqual("blocked", production_replacement_worklist["submission_status"])
                self.assertEqual(72, production_replacement_worklist["submitted_task_count"])
                self.assertEqual(72, production_replacement_worklist["submitted_placeholder_source_uri_count"])
                self.assertEqual(3, len(production_replacement_worklist["submitted_task_refs"]))
                self.assertEqual(3, len(production_replacement_worklist["submitted_tasks"]))
                self.assertEqual("placeholder", production_replacement_worklist["submitted_tasks"][0]["source_uri_status"])
                self.assertEqual(3, production_replacement_worklist["submitted_task_counts_by_requirement"]["oss-verifier-and-public-spec"])
                self.assertEqual(72, production_replacement_worklist["submitted_task_total"])
                self.assertEqual(72, sum(production_replacement_worklist["submitted_task_counts_by_owner"].values()))
                self.assertEqual(72, sum(production_replacement_worklist["submitted_task_counts_by_authority_kind"].values()))
                self.assertTrue(production_replacement_worklist["submission_review_present"])
                self.assertFalse(production_replacement_worklist["submission_review_ready"])
                self.assertEqual("blocked", production_replacement_worklist["submission_review_status"])
                self.assertEqual(72, production_replacement_worklist["submission_review_blocked_task_count"])
                self.assertEqual(72, production_replacement_worklist["submission_review_placeholder_source_uri_count"])
                self.assertTrue(production_replacement_worklist["remediation_queue_present"])
                self.assertEqual("blocked", production_replacement_worklist["remediation_queue_status"])
                self.assertEqual(72, production_replacement_worklist["remediation_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_placeholder_source_uri_count"])
                self.assertEqual(3, len(production_replacement_worklist["remediation_tasks"]))
                self.assertIn("replace source_uri", production_replacement_worklist["remediation_tasks"][0]["remediation_action"])
                self.assertEqual(72, sum(production_replacement_worklist["remediation_task_counts_by_owner"].values()))
                self.assertEqual(72, sum(production_replacement_worklist["remediation_task_counts_by_authority_kind"].values()))
                self.assertTrue(production_replacement_worklist["remediation_owner_packet_bundle_present"])
                self.assertEqual("blocked", production_replacement_worklist["remediation_owner_packet_bundle_status"])
                self.assertEqual(10, production_replacement_worklist["remediation_owner_packet_count"])
                self.assertEqual(10, production_replacement_worklist["remediation_owner_packet_owner_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_owner_packet_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_owner_packet_blocked_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_owner_packet_placeholder_source_uri_count"])
                self.assertEqual(72, sum(production_replacement_worklist["remediation_owner_packet_counts_by_owner"].values()))
                self.assertEqual(3, len(production_replacement_worklist["remediation_owner_packets"]))
                self.assertEqual(10, production_replacement_worklist["remediation_owner_packet_total"])
                self.assertIn("owner_hint", production_replacement_worklist["remediation_owner_packets"][0])
                self.assertIn("authority_kinds", production_replacement_worklist["remediation_owner_packets"][0])
                self.assertTrue(production_replacement_worklist["remediation_owner_fulfillment_template_present"])
                self.assertEqual("blocked", production_replacement_worklist["remediation_owner_fulfillment_template_status"])
                self.assertEqual(10, production_replacement_worklist["remediation_owner_fulfillment_template_owner_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_owner_fulfillment_template_fulfillment_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_owner_fulfillment_template_placeholder_source_uri_count"])
                self.assertEqual(0, production_replacement_worklist["remediation_owner_fulfillment_template_live_source_uri_count"])
                self.assertEqual(3, len(production_replacement_worklist["remediation_owner_fulfillments"]))
                self.assertEqual(72, production_replacement_worklist["remediation_owner_fulfillment_total"])
                self.assertTrue(production_replacement_worklist["remediation_application_present"])
                self.assertFalse(production_replacement_worklist["remediation_application_ready"])
                self.assertEqual("blocked", production_replacement_worklist["remediation_application_status"])
                self.assertEqual("blocked", production_replacement_worklist["remediation_application_applied_review_status"])
                self.assertEqual(72, production_replacement_worklist["remediation_application_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_application_applied_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_application_blocked_task_count"])
                self.assertEqual(72, production_replacement_worklist["remediation_application_placeholder_source_uri_count"])
                ...
                authority_evidence=[
                    {
                        ...
                        "evidence_hash": "sha256:0663865319edab9e568e99299b7e142e1db5b811a77db28843cde686d1b63d00",
                    ...
                    },
                    {
                        ...
                        "evidence_hash": "sha256:9ff48181c6894f37af335e7f5ad4093145009574615b1aa663c705db081cd488",
            ...
            authority_evidence = [
                {
                    ...
                    "evidence_hash": "sha256:531d0507835b71ab71b64d88c0c1795c3cbba9aa558f893a83df3b3e8e7a00cc",
                ...
                },
                {
                    ...
                    "evidence_hash": "sha256:051f673ef66cd0c298f2a48f53fb0ead826bfecf6b0208b58908bbbc93ffd995",
